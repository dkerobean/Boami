 ...
 * Retrieves income categories for the authenticated user
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Income categories GET error:', error);
    return NextResponse.json(
      { success: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to retrieve income categories' } },
      { status: 500 }
    );
  }
}
...
/**
 * POST /api/finance/categories/income
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Income category POST error:', error);
    ...
    return NextResponse.json(
      { success: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to create income category' } },
      { status: 500 }
    );
  }
}